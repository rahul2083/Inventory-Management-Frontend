...
export default function Dispatch({
  ...
  const dashboardStats = useMemo(() => {
    ...
    return { totalDispatch, readyCount, inTransitCount, deliveredCount, rtoCount, totalRevenue, totalFreight, totalPackagingCost };
  }, [activeDispatches, inTransitDispatches, deliveredDispatches, rtoDispatches]);
  ...
  }, [activeDispatches, deliveredDispatches, rtoDispatches, cancelledDispatches, inTransitDispatches, activeTabView, searchTerm, platformFilter, getDetails]);
  ...
  const downloadGatepass = async (orderGuid, orderId) => {
    try {
      const res = await api.get(`/gatepass/${orderGuid}`, { responseType: "blob" });
      const url  = window.URL.createObjectURL(new Blob([res.data], { type: "application/pdf" }));
      const link = document.createElement("a");
      link.href  = url;
      link.download = `gatepass-${orderId || orderGuid}.pdf`;
      link.click();
  ...
  const isGeMDelivery = Array.isArray(logisticsBatch)
    && logisticsBatch.length > 0
    && logisticsBatch.every((item) => String(item.firmName || "").trim().toLowerCase() === "gem");
  ...
  const handleSaveLogistics = async (e) => {
    ...
    try {
      ...
      if (isGeMDelivery && finalLogisticsStatus === "Delivered") {
      ...
      // Upload DPOD for GeM orders if a file is attached
      ...
      {logisticsBatch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSaveLogistics} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" className="w-full border p-2.5 rounded-xl text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Enter tracking number" value={logisticsForm.trackingId} onChange={(e) => setLogisticsForm({...logisticsForm, trackingId: e.target.value})} disabled={isDeliveredLogisticsLocked} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              {isGeMDelivery && logisticsForm.logisticsStatus === "Delivered" && (
                <div className="p-4 bg-orange-50 rounded-2xl border border-orange-100">
                  ...
                  />
                  <p className="text-xs text-orange-600 mt-2">Required for GeM orders.</p>
                </div>
              )}
