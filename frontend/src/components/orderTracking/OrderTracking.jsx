...
export default function OrderTracking({
  ...
  const handleRemoveSerial = async (dispatchGuid, serialValue) => {
    if (!isAdmin && !isSupervisor) {
      showToast("Access denied: Admin/Supervisor only", "error");
      return;
    }
    ...
    try {
      ...
      await printerService.deleteDispatch([dispatchGuid], "Removed from order manually", currentUser?.username || "Admin");
                ...
                currentBatches.map((batch, index) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`group transition-all hover:shadow-md ${!tagBgColor ? (colorClass || (batch.rowColor && !batch.rowColor.includes('|') ? batch.rowColor : 'hover:bg-slate-50')) : ''} ${hasActiveReturns && !tagBgColor ? "bg-red-50/30" :
      ...
      {modalOpen && selectedBatch && (
        <OrderDetailModal
          {...{
            ...
            extraDocInputRef, extraDocType, handleRemoveSerial, handleReplaceSerial,
            handleRestoreBatch, handleSaveEdits, handleSavePaymentEdit,
            handleToggleInstallation, handleUpdateStatus, handleUploadExtraDoc,
            handleViewDocument, isAdmin, isEditMode, isEditingPayment, isSupervisor,
            isUpdating, localSerials, modalDetailTab, newStatus, paymentEditForm,
            replaceWithSerialId, replacingItemId, restoringBatchKey, returns,
            selectedBatch, setCancellationReason, setContractFile, setEditFormData,
            ...
            setTrackingId, trackingId, uploadingContract, uploadingExtraDoc,
          }}
        />
      )}
      <AppearanceModal
