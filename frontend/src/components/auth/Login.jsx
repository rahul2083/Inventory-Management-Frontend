...
import { Loader2, ShieldCheck, Database, Layers, LayoutPanelLeft, Truck, Cpu } from "lucide-react";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await printerService.login(credentials);

      // If the server returns HTML instead of JSON (e.g. 200 OK but holding page), result.user will be undefined.
      ...
      navTimer = setTimeout(() => { navigate("/"); }, 1500);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f4f7f6] via-[#e2e8f0] to-[#cbd5e1] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans relative overflow-hidden">
      ...
      <div className="max-w-6xl w-full flex flex-col lg:flex-row items-center gap-12 relative z-10">
        ...
        {/* LEFT SECTION: Enterprise Text (New Addition) */}
        <div className="lg:w-1/2 text-left space-y-6 animate__animated animate__fadeInLeft">
          ...
          <div className="text-slate-600 text-base lg:text-lg max-w-md leading-relaxed font-medium space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Streamlines inventory tracking</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green-500">✔</span>
              <span>Simplifies order processing</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green-500">✔</span>
              <span>Manages returns efficiently</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green-500">✔</span>
              <span>Reduces operational delays</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green-500">✔</span>
              <span>Improves stock visibility</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green-500">✔</span>
              <span>Enhances overall workflow</span>
            </div>
        ...
        {/* RIGHT SECTION: Existing Login Card */}
        <div className="lg:w-1/2 w-full max-w-md">
          <div id="login_sec" className="bg-white/90 backdrop-blur-md p-10 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/50 animate__animated animate__zoomIn">

            {/* Header Section */}
            <div className="flex justify-between items-center mb-10 w-full">
              <img
                src="/aplus.png"
                className="w-48 h-auto animate__animated animate__heartBeat"
                alt="Logo"
                onError={(e) => { e.target.style.display = 'none'; }}
              />
              <h4 className="text-2xl font-black text-slate-800 tracking-tight animate__animated animate__backInRight">
                LOG IN
              </h4>
            </div>

            {/* Form Section */}
            <form onSubmit={handleSubmit} className="space-y-6 animate__animated animate__backInUp">
          ...
          {/* Footer Branding inside the right side */}
          <div className="mt-8 text-center animate__animated animate__fadeIn animate__delay-1s">
